...
// inputs: xp      horizontal position of bottom left corner of image, columns from the left edge
//                     must be less than 84
//                     0 is on the left; 82 is near the right
//         yp     vertical position of bottom left corner of image, rows from the top edge
...
// outputs: none
void Nokia5110_PrintBMP(unsigned char xp, unsigned char yp, const unsigned char *ptr, unsigned char threshold){
  ...
  if((height <= 0) ||              // bitmap is unexpectedly encoded in top-to-bottom pixel order
     ...
     ((xp + width) > SCREENW) || // right side cut off
     (yp < (height - 1)) ||      // top cut off
     (yp > SCREENH))           { // bottom cut off
    return;
  }
  if(threshold > 14){
  ...
  screeny = yp/8;
  screenx = xp + SCREENW*screeny;
  mask = yp%8;                // row 0 to 7
